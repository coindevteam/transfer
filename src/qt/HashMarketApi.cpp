 ...
 *
 * AIM : Update exchanges last prices for TWC market when requested
 *
 * Overview screen will call refresh and connected using signal.
 *
 * On the overview page we are pulling current prices from the markets to show the current price for TWC.
 *
 */
...
/*
Mintpal
https://api.mintpal.com/v2/market/stats/TWC/BTC
we want to pull the info from “last_price”
https://api.mintpal.com/v2/market/stats/AUR/BTC is an example as the market TWC has not been created.
*/
...
https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-aur is an example as the market TWC has not been created.
*/
...
https://c-cex.com/t/btc-usd.json is an example as the market TWC has not been created.
*/
...
//    market = "usd";
    ...
    QUrl url( QString("https://c-cex.com/t/pyra-btc.json").arg(market) );
    QNetworkReply* reply = nam->get(QNetworkRequest(url));

    QObject::connect(reply, SIGNAL(finished()), this, SLOT(ccexFinished()));
}
...
we need to pull info from line BTC_TWC
